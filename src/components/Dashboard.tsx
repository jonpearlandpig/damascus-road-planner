      ...
      <section className="dashboard-section">
        ...
        <div className="venue-grid">{venues.map((venue, index) => (
          <article key={venue.slug} className={`venue-card ${venue.sourceStatus === 'MISSING' ? 'venue-card--missing' : ''}`}>
            ...
            <div className="venue-card__score"><div className="score-ring" style={{ '--score': `${venue.sourceScore * 3.6}deg` } as CSSProperties}><span>{venue.sourceScore}</span></div><div><span>Source score</span><strong>{venue.sourceYear}</strong><small>{venue.cadStatus === 'NONE' ? 'No native CAD' : 'CAD requested'}</small></div></div>
