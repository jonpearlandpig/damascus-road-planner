# CLAUDE.md ADDITIONS — damascus-road-planner
Append to the repo's agent rules file (create CLAUDE.md if absent; AGENTS.md defers to it).
...
- One job per session; work on `agent/<topic>` branches; CI (build+lint) must pass
  before merge to main. Codex reviews diffs/PRs; Claude Code builds. Never both on
